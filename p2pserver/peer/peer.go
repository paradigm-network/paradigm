...
import (
	...
	conn "github.com/paradigm-network/paradigm/p2pserver/link"
...
func rmPeer(p *Peer) {
	...
	logger := log.GetLogger("peer rmPeer()")

	logger.Debug().Msgf("[p2p]Remove unused peer: %d", p.GetID())
}

//DumpInfo print all information of peer
func (this *Peer) DumpInfo() {

	logger := log.GetLogger("peer DumpInfo()")

	logger.Debug().Msg("[p2p]Node info:")
	logger.Debug().Msgf("[p2p]\t syncState = ", this.syncState)
	logger.Debug().Msgf("[p2p]\t consState = ", this.consState)
	logger.Debug().Msgf("[p2p]\t id = ", this.GetID())
	logger.Debug().Msgf("[p2p]\t addr = ", this.SyncLink.GetAddr())
	logger.Debug().Msgf("[p2p]\t cap = ", this.cap)
	logger.Debug().Msgf("[p2p]\t version = ", this.GetVersion())
	logger.Debug().Msgf("[p2p]\t services = ", this.GetServices())
	logger.Debug().Msgf("[p2p]\t syncPort = ", this.GetSyncPort())
	logger.Debug().Msgf("[p2p]\t consPort = ", this.GetConsPort())
	logger.Debug().Msgf("[p2p]\t relay = ", this.GetRelay())
	logger.Debug().Msgf("[p2p]\t height = ", this.GetHeight())
}
...
func (this *Peer) GetAddr16() ([16]byte, error) {
	...
	logger := log.GetLogger("peer GetAddr16()")
	...
	if ip == nil {
		logger.Warn().Msgf("[p2p]parse ip address error\n", this.GetAddr())
