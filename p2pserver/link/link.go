...
var logger = log.GetLogger("message link")
...
func (this *Link) Rx() {
	...
	for {
		...
		if err != nil {
			logger.Info().Msgf("[p2p]error read from %s :%s", this.GetAddr(), err.Error())
			break
		}
		...
		if !this.needSendMsg(msg) {
			logger.Debug().Msgf("skip handle msgType:%s from:%d", msg.CmdType(), this.id)
			continue
		}
...
func (this *Link) disconnectNotify() {
	logger.Debug().Msgf("[p2p]call disconnectNotify for %s", this.GetAddr())
	this.CloseConn()
...
func (this *Link) Tx(msg types.Message) error {
	...
	if err != nil {
		logger.Debug().Msgf("[p2p]error serialize messge ", err.Error())
		return err
	}

	payload := sink.Bytes()
	nByteCnt := len(payload)
	logger.Info().Msgf("[p2p]TX buf length: %d\n", nByteCnt)
	...
	if err != nil {
		logger.Info().Msgf("[p2p]error sending messge to %s :%s", this.GetAddr(), err.Error())
		this.disconnectNotify()
		return err
	}
