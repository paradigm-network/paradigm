...
import (
	...
	"github.com/paradigm-network/paradigm/p2pserver/message/utils"
	"github.com/paradigm-network/paradigm/p2pserver/net/netserver"
...
func (this *P2PServer) Xmit(message interface{}) error {
	...
	default:
		logger.Warn().Msgf("[p2p]Unknown Xmit message %v , type %v", message,
...
func (this *P2PServer) Send(p *peer.Peer, msg msgtypes.Message,
	...
	logger.Warn().Msgf("[p2p]send to a not ESTABLISH peer %d",
		p.GetID())
...
func (this *P2PServer) WaitForSyncBlkFinish() {
	consensusType := strings.ToLower(config.DefConfig.GenesisConfig.ConsensusType)
	...
	for {
		...
		logger.Info().Msgf("[p2p]WaitForSyncBlkFinish... current block height is ",
...
func (this *P2PServer) WaitForPeersStart() {
	...
	for {
		logger.Info().Msgf("[p2p]Wait for minimum connection...")
...
func (this *P2PServer) connectSeeds() {
	...
	for _, n := range config.DefConfig.GenesisConfig.SeedList {
		ip, err := common.ParseIPAddr(n)
		if err != nil {
			logger.Warn().Msgf("[p2p]seed peer %s address format is wrong", n)
			continue
		}
		ns, err := net.LookupHost(ip)
		if err != nil {
			logger.Warn().Msgf("[p2p]resolve err: %s", err.Error())
			continue
		}
...
func (this *P2PServer) retryInactivePeer() {
	...
	for _, p := range np.List {
		...
		if p.GetSyncState() == common.INACTIVITY {
			logger.Debug().Msgf("[p2p] try reconnect %s", nodeAddr)
	...
	if connCount >= config.DefConfig.P2PNodeConfig.MaxConnOutBound {
		logger.Warn().Msgf("[p2p]Connect: out connections(%d) reach the max limit(%d)", connCount,
			config.DefConfig.P2PNodeConfig.MaxConnOutBound)
		return
	}
	...
	if len(this.RetryAddrs) > 0 {
		...
		for _, addr := range addrs {
			rand.Seed(time.Now().UnixNano())
			logger.Debug().Msgf("[p2p]Try to reconnect peer, peer addr is ", addr)
			<-time.After(time.Duration(rand.Intn(common.CONN_MAX_BACK)) * time.Millisecond)
			logger.Debug().Msgf("[p2p]Back off time`s up, start connect node")
	...
	for _, p := range peers {
		if p.GetSyncState() == common.ESTABLISH {
			...
			if t.Before(time.Now().Add(-1 * time.Second *
				...
				logger.Warn().Msgf("[p2p]keep alive timeout!!!lost remote peer %d - %s from %s", p.GetID(), p.SyncLink.GetAddr(), t.String())
				p.CloseSync()
				p.CloseCons()
			}
		}
...
func (this *P2PServer) tryRecentPeers() {
	netID := config.DefConfig.P2PNodeConfig.NetworkMagic
	if comm.FileExisted(common.RECENT_FILE_NAME) {
		buf, err := ioutil.ReadFile(common.RECENT_FILE_NAME)
		if err != nil {
			logger.Warn().Msgf("[p2p]read %s fail:%s, connect recent peers cancel", common.RECENT_FILE_NAME, err.Error())
			return
		}

		err = json.Unmarshal(buf, &this.recentPeers)
		if err != nil {
			logger.Warn().Msgf("[p2p]parse recent peer file fail: ", err)
			return
		}
		if len(this.recentPeers[netID]) > 0 {
			logger.Info().Msgf("[p2p]try to connect recent peer")
		}
...
func (this *P2PServer) syncPeerAddr() {
	...
	if changed {
		...
		if err != nil {
			logger.Warn().Msgf("[p2p]package recent peer fail: ", err)
			return
		}
		err = ioutil.WriteFile(common.RECENT_FILE_NAME, buf, os.ModePerm)
		if err != nil {
			logger.Warn().Msgf("[p2p]write recent peer fail: ", err)
		}
	}
}
