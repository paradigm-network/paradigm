...
import (
	...
	evtActor "github.com/paradigm-network/paradigm/network/actor"
	...
	"github.com/paradigm-network/paradigm/core/ledger"
	"github.com/paradigm-network/paradigm/core/types"
	actor "github.com/paradigm-network/paradigm/p2pserver/actor/req"
	msgCommon "github.com/paradigm-network/paradigm/p2pserver/common"
	"github.com/paradigm-network/paradigm/p2pserver/message/msg_pack"
	msgTypes "github.com/paradigm-network/paradigm/p2pserver/message/types"
...
var logger = log.GetLogger("message utils")
...
func AddrReqHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive addr request message", data.Addr, data.Id)
	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		logger.Debug().Msg("[p2p]remotePeer invalid in AddrReqHandle")
		return
	}
	...
	//check mask peers
	mskPeers := config.DefConfig.P2PNodeConfig.ReservedCfg.MaskPeers
	if config.DefConfig.P2PNodeConfig.ReservedPeersOnly && len(mskPeers) > 0 {
...
func HeadersReqHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive headers request message", data.Addr, data.Id)
	...
	if err != nil {
		logger.Warn().Msgf("get headers in HeadersReqHandle error: %s,startHash:%s,stopHash:%s", err.Error(), startHash.ToHexString(), stopHash.ToHexString())
		return
	}
	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		logger.Debug().Msgf("[p2p]remotePeer invalid in HeadersReqHandle, peer id: %d", data.Id)
		return
	}
...
func PingHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive ping message", data.Addr, data.Id)

	ping := data.Payload.(*msgTypes.Ping)
	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		logger.Debug().Msgf("[p2p]remotePeer invalid in PingHandle")
		return
	}
	remotePeer.SetHeight(ping.Height)

	//height := ledger.DefLedger.GetCurrentBlockHeight()
	p2p.SetHeight(uint64(blockheight))
	msg := msgpack.NewPongMsg(uint64(blockheight))
...
func PongHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive pong message", data.Addr, data.Id)

	pong := data.Payload.(*msgTypes.Pong)

	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		logger.Debug().Msgf("[p2p]remotePeer invalid in PongHandle")
		return
	}
...
func BlkHeaderHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive block header message", data.Addr, data.Id)
	if pid != nil {
...
func BlockHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive block message from ", data.Addr, data.Id)
...
func ConsensusHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Debug().Msgf("[p2p]receive consensus message:%v,%d", data.Addr, data.Id)
...
func NotFoundHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	logger.Debug().Msgf("[p2p]receive notFound message, hash is ", notFound.Hash)
}
...
func TransactionHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive transaction message", data.Addr, data.Id)

	var trn = data.Payload.(*msgTypes.Trn)
	actor.AddTransaction(trn.Txn)
	logger.Info().Msgf("[p2p]receive Transaction message hash", trn.Txn.Hash())
...
func VersionHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive version message", data.Addr, data.Id)

	version := data.Payload.(*msgTypes.Version)

	remotePeer := p2p.GetPeerFromAddr(data.Addr)
	if remotePeer == nil {
		logger.Debug().Msgf("[p2p]peer is not exist", data.Addr)
	...
	if config.DefConfig.P2PNodeConfig.ReservedPeersOnly && len(config.DefConfig.P2PNodeConfig.ReservedCfg.ReservedPeers) > 0 {
		found := false
		for _, addr := range config.DefConfig.P2PNodeConfig.ReservedCfg.ReservedPeers {
			if strings.HasPrefix(data.Addr, addr) {
				logger.Debug().Msgf("[p2p]peer in reserved list", data.Addr)
				found = true
				break
			}
		}
		if !found {
			...
			logger.Debug().Msgf("[p2p]peer not in reserved list,close", data.Addr)
			return
		}
	...
	if version.P.IsConsensus == true {
		if config.DefConfig.P2PNodeConfig.DualPortSupport == false {
			logger.Warn().Msgf("[p2p]consensus port not surpport", data.Addr)
			remotePeer.CloseCons()
			return
		}
		...
		if p == nil {
			logger.Warn().Msgf("[p2p]sync link is not exist", version.P.Nonce, data.Addr)
			remotePeer.CloseCons()
			remotePeer.CloseSync()
			return
		} else {
		...
		if version.P.Nonce == p2p.GetID() {
			logger.Warn().Msgf("[p2p]the node handshake with itself", data.Addr)
		...
		if s != msgCommon.INIT && s != msgCommon.HAND {
			logger.Warn().Msgf("[p2p]unknown status to received version,%d,%s\n", s, data.Addr)
			remotePeer.CloseCons()
			return
		}
		...
		if s == msgCommon.INIT {
			...
			msg = msgpack.NewVersion(p2p, true, blockheight)
		...
		if version.P.Nonce == p2p.GetID() {
			p2p.RemoveFromInConnRecord(remotePeer.GetAddr())
			p2p.RemoveFromOutConnRecord(remotePeer.GetAddr())
			logger.Warn().Msgf("[p2p]the node handshake with itself", remotePeer.GetAddr())
			p2p.SetOwnAddress(nodeAddr)
			remotePeer.CloseSync()
			return
		}
		...
		if p != nil {
			...
			if err != nil {
				logger.Warn().Msgf("[p2p]exist peer %d ip format is wrong %s", version.P.Nonce, p.GetAddr())
				return
			}
			ipNew, err := msgCommon.ParseIPAddr(data.Addr)
			if err != nil {
				remotePeer.CloseSync()
				logger.Warn().Msgf("[p2p]connecting peer %d ip format is wrong %s, close", version.P.Nonce, data.Addr)
				return
			}
			if ipNew == ipOld {
				//same id and same ip
				...
				if ret == true {
					logger.Info().Msgf("[p2p]peer reconnect %d", version.P.Nonce, data.Addr)
				...
				logger.Warn().Msgf("[p2p]same peer id from different addr: %s, %s close latest one", ipOld, ipNew)
				remotePeer.CloseSync()
				return
		...
		if s == msgCommon.INIT {
			...
			msg = msgpack.NewVersion(p2p, false, blockheight)
		} else if s == msgCommon.HAND {
...
func VerAckHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive verAck message from ", data.Addr, data.Id)

	verAck := data.Payload.(*msgTypes.VerACK)
	remotePeer := p2p.GetPeer(data.Id)

	if remotePeer == nil {
		logger.Warn().Msgf("[p2p]nbr node is not exist", data.Id, data.Addr)
		return
	}

	if verAck.IsConsensus == true {
		if config.DefConfig.P2PNodeConfig.DualPortSupport == false {
			logger.Warn().Msgf("[p2p]consensus port not surpport")
			return
		}
		s := remotePeer.GetConsState()
		if s != msgCommon.HAND_SHAKE && s != msgCommon.HAND_SHAKED {
			logger.Warn().Msgf("[p2p]unknown status to received verAck,state:%d,%s\n", s, data.Addr)
			return
		}
		...
		if s == msgCommon.HAND_SHAKE {
			...
			if config.DefConfig.P2PNodeConfig.DualPortSupport && remotePeer.GetConsPort() > 0 {
...
func AddrHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]handle addr message", data.Addr, data.Id)
	...
	for _, v := range msg.NodeAddrs {
		...
		logger.Debug().Msgf("[p2p]connect ip address:", address)
...
func DataReqHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive data req message", data.Addr, data.Id)

	var dataReq = data.Payload.(*msgTypes.DataReq)

	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		logger.Debug().Msgf("[p2p]remotePeer invalid in DataReqHandle")
		return
	}
	...
	case common.BLOCK:
		...
		if block == nil {
			...
			if err != nil || block == nil || block.Header == nil {
				logger.Debug().Msgf("[p2p]can't get block by hash: ", hash,
		...
		logger.Debug().Msgf("[p2p]block height is ", block.Header.Height,
			" ,hash is ", hash)
	...
	case common.TRANSACTION:
		...
		if err != nil {
			logger.Debug().Msgf("[p2p]Can't get transaction by hash: ",
...
func InvHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Info().Msgf("[p2p]receive inv message", data.Addr, data.Id)
	var inv = data.Payload.(*msgTypes.Inv)

	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		logger.Debug().Msgf("[p2p]remotePeer invalid in InvHandle")
		return
	}
	if len(inv.P.Blk) == 0 {
		logger.Debug().Msgf("[p2p]empty inv payload in InvHandle")
		return
	}
	var id common.Uint256
	str := inv.P.Blk[0].ToHexString()
	logger.Debug().Msgf("[p2p]the inv type: 0x%x block len: %d, %s\n",
		inv.P.InvType, len(inv.P.Blk), str)

	invType := common.InventoryType(inv.P.InvType)
	switch invType {
	case common.TRANSACTION:
		logger.Debug().Msgf("[p2p]receive transaction message", id)
	...
	case common.BLOCK:
		logger.Debug().Msgf("[p2p]receive block message")
		for _, id = range inv.P.Blk {
			logger.Debug().Msgf("[p2p]receive inv-block message, hash is ", id)
			// TODO check the ID queue
			...
			if !isContainBlock && msgTypes.LastInvHash != id {
				...
				logger.Info().Msgf("[p2p]inv request block hash: %x", id)
	...
	case common.CONSENSUS:
		logger.Debug().Msgf("[p2p]receive consensus message")
		id = inv.P.Blk[0]
	...
	default:
		logger.Warn().Msgf("[p2p]receive unknown inventory message")
	}
...
func DisconnectHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	logger.Debug().Msgf("[p2p]receive disconnect message", data.Addr, data.Id)
	p2p.RemoveFromInConnRecord(data.Addr)
	p2p.RemoveFromOutConnRecord(data.Addr)
	remotePeer := p2p.GetPeer(data.Id)
	if remotePeer == nil {
		logger.Debug().Msgf("[p2p]disconnect peer is nil")
		return
	}
	p2p.RemoveFromConnectingList(data.Addr)
...
func GetHeadersFromHash(startHash common.Uint256, stopHash common.Uint256) ([]*types.Header, error) {
	...
	for i = 1; i <= count; i++ {
		...
		if err != nil {
			logger.Debug().Msgf("[p2p]net_server GetBlockWithHeight failed with err=%s, hash=%x,height=%d\n", err.Error(), hash, stopHeight+i)
			return nil, err
		}
