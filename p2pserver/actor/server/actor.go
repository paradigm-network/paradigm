...
import (
	...
	"github.com/paradigm-network/paradigm/p2pserver"
	"github.com/paradigm-network/paradigm/p2pserver/common"
)


var logger = log.GetLogger("actor server")
...
func (this *P2PActor) Receive(ctx actor.Context) {
	...
	case *actor.Restarting:
		logger.Warn().Msgf("[p2p]actor restarting")
	case *actor.Stopping:
		logger.Warn().Msgf("[p2p]actor stopping")
	case *actor.Stopped:
		logger.Warn().Msgf("[p2p]actor stopped")
	case *actor.Started:
		logger.Warn().Msgf("[p2p]actor started")
	case *actor.Restart:
		logger.Warn().Msgf("[p2p]actor restart")
	case *StopServerReq:
	...
	default:
		...
		if nil != err {
			logger.Warn().Msgf("[p2p]error xmit message ", err.Error(), reflect.TypeOf(ctx.Message()))
		}
	}
}
...
func (this *P2PActor) handleTransmitConsensusMsgReq(ctx actor.Context, req *TransmitConsensusMsgReq) {
	...
	if peer != nil {
		...
		logger.Warn().Msgf("[p2p]can`t transmit consensus msg:no valid neighbor peer: %d\n", req.Target)
	}
}
