...
func (this *NetServer) init() error {
	...
	logger := log.GetLogger("netserver init()")

	this.base.SetVersion(common.PROTOCOL_VERSION)

	if config.DefConfig.ConsensusConfig.EnableConsensus {
		this.base.SetServices(uint64(common.VERIFY_NODE))
	} else {
		this.base.SetServices(uint64(common.SERVICE_NODE))
	}

	if config.DefConfig.P2PNodeConfig.NodePort == 0 {
		logger.Error().Msg("[p2p]link port invalid")
		return errors.New("[p2p]invalid link port")
	}

	this.base.SetSyncPort(uint16(config.DefConfig.P2PNodeConfig.NodePort))

	if config.DefConfig.P2PNodeConfig.DualPortSupport {
		if config.DefConfig.P2PNodeConfig.NodeConsensusPort == 0 {
			logger.Error().Msg("[p2p]consensus port invalid")
			return errors.New("[p2p]invalid consensus port")
		}

		this.base.SetConsPort(uint16(config.DefConfig.P2PNodeConfig.NodeConsensusPort))
	} else {
	...
	this.base.SetID(id)
	logger.Info().Msgf("[p2p]init peer ID to",this.base.GetID())
...
func (this *NetServer) Send(p *peer.Peer, msg types.Message, isConsensus bool) error {
	...
	logger := log.GetLogger("netserver Send()")
	...
	logger.Warn().Msg("[p2p]send to a invalid peer")
...
func (this *NetServer) Connect(addr string, isConsensus bool) error {
	logger := log.GetLogger("netserver Connect()")

	if this.IsAddrInOutConnRecord(addr) {

		logger.Debug().Msgf("[p2p]Address: %s Consensus: %v is in OutConnectionRecord,",addr,isConsensus)
	...
	if connCount >= config.DefConfig.P2PNodeConfig.MaxConnOutBound {
		logger.Warn().Msgf("[p2p]Connect: out connections(%d) reach the max limit(%d)",connCount,config.DefConfig.P2PNodeConfig.MaxConnOutBound)
	...
	if added := this.AddOutConnectingList(addr); added == false {
		logger.Debug().Msgf("[p2p]node exist in connecting list",addr)
	}
	this.connectLock.Unlock()
	...
	if isTls {
		...
		if err != nil {
			...
			logger.Debug().Msgf("[p2p]connect %s failed:%s",addr,err.Error())
			return err
		}
	} else {
	...
	logger.Debug().Msgf("[p2p]peer %s connect with %s with %s",
	...
	version := msgpack.NewVersion(this, isConsensus,blockheight)
...
func (this *NetServer) startListening() error {
	logger := log.GetLogger("netserver startListening()")
	var err error
	...
	if syncPort == 0 {
		logger.Error().Msg("[p2p]sync port invalid")
		return errors.New("[p2p]sync port invalid")
	}

	err = this.startSyncListening(syncPort)
	if err != nil {
		logger.Error().Msg("[p2p]start sync listening fail")
		return err
	}

	//consensus
	if config.DefConfig.P2PNodeConfig.DualPortSupport == false {
		logger.Debug().Msg("[p2p]dual port mode not supported,keep single link")
		return nil
	}
	if consPort == 0 || consPort == syncPort {
		//still work
		logger.Warn().Msg("[p2p]consensus port invalid,keep single link")
	} else {
...
func (this *NetServer) startSyncListening(port uint16) error {
	logger := log.GetLogger("netserver startSyncListening()")
	var err error
	this.synclistener, err = createListener(port)
	if err != nil {
		logger.Error().Msg("[p2p]failed to create sync listener")
		return errors.New("[p2p]failed to create sync listener")
	}

	go this.startSyncAccept(this.synclistener)
	logger.Info().Msgf("[p2p]start listen on sync port %d", port)
	return nil
}
...
func (this *NetServer) startConsListening(port uint16) error {
	logger := log.GetLogger("netserver startConsListening()")
	var err error
	this.conslistener, err = createListener(port)
	if err != nil {
		logger.Error().Msg("[p2p]failed to create cons listener")
		return errors.New("[p2p]failed to create cons listener")
	}

	go this.startConsAccept(this.conslistener)
	logger.Info().Msgf("[p2p]Start listen on consensus port %d", port)
	return nil
}
...
func (this *NetServer) startSyncAccept(listener net.Listener) {
	...
	logger := log.GetLogger("netserver startSyncAccept()")

	for {
		conn, err := listener.Accept()

		if err != nil {
			logger.Error().Msgf("[p2p]error accepting ", err.Error())
			return
		}

		logger.Debug().Msgf("[p2p]remote sync node connect with ",
			conn.RemoteAddr(), conn.LocalAddr())
		if !this.AddrValid(conn.RemoteAddr().String()) {
			logger.Warn().Msgf("[p2p]remote %s not in reserved list, close it ", conn.RemoteAddr())
			conn.Close()
			continue
		}
		...
		if syncAddrCount >= config.DefConfig.P2PNodeConfig.MaxConnInBound {
			logger.Warn().Msgf("[p2p]SyncAccept: total connections(%d) reach the max limit(%d), conn closed",
				syncAddrCount, config.DefConfig.P2PNodeConfig.MaxConnInBound)
			conn.Close()
			continue
		}
		...
		if err != nil {
			logger.Warn().Msgf("[p2p]parse ip error ", err.Error())
			conn.Close()
			continue
		}
		connNum := this.GetIpCountInInConnRecord(remoteIp)
		if connNum >= config.DefConfig.P2PNodeConfig.MaxConnInBoundForSingleIP {
			logger.Warn().Msgf("[p2p]SyncAccept: connections(%d) with ip(%s) has reach the max limit(%d), "+
				"conn closed", connNum, remoteIp, config.DefConfig.P2PNodeConfig.MaxConnInBoundForSingleIP)
			conn.Close()
			continue
		}
...
func (this *NetServer) startConsAccept(listener net.Listener) {
	...
	logger := log.GetLogger("netserver startConsAccept()")
	...
	for {
		...
		logger.Debug().Msgf("[p2p]remote cons node connect with ",
			conn.RemoteAddr(), conn.LocalAddr())
		if !this.AddrValid(conn.RemoteAddr().String()) {
			logger.Warn().Msgf("[p2p]remote %s not in reserved list, close it ", conn.RemoteAddr())
			conn.Close()
...
func (this *NetServer) AddOutConnectingList(addr string) (added bool) {
	...
	logger := log.GetLogger("netserver AddOutConnectingList()")
	...
	logger.Info().Msgf("[p2p]add to out connecting list", addr)
...
func (this *NetServer) RemoveFromConnectingList(addr string) {
	...
	logger := log.GetLogger("netserver RemoveFromConnectingList()")
	...
	logger.Info().Msgf("[p2p]remove from out connecting list", addr)
...
func (this *NetServer) AddPeerSyncAddress(addr string, p *peer.Peer) {
	...
	logger := log.GetLogger("netserver AddPeerSyncAddress()")

	this.PeerAddrMap.Lock()
	defer this.PeerAddrMap.Unlock()
	logger.Debug().Msgf("[p2p]AddPeerSyncAddress %s", addr)
...
func (this *NetServer) AddPeerConsAddress(addr string, p *peer.Peer) {
	...
	logger := log.GetLogger("netserver AddPeerConsAddress()")

	this.PeerAddrMap.Lock()
	defer this.PeerAddrMap.Unlock()
	logger.Debug().Msgf("[p2p]AddPeerConsAddress %s", addr)
...
func (this *NetServer) AddInConnRecord(addr string) {
	logger := log.GetLogger("netserver AddInConnRecord()")
	...
	logger.Debug().Msgf("[p2p]add in record  %s", addr)
}
...
func (this *NetServer) RemoveFromInConnRecord(addr string) {
	logger := log.GetLogger("netserver RemoveFromInConnRecord()")
	this.inConnRecord.Lock()
	defer this.inConnRecord.Unlock()
	addrs := []string{}
	for _, a := range this.inConnRecord.InConnectingAddrs {
	...
	logger.Debug().Msgf("[p2p]remove in record  %s", addr)
...
func (this *NetServer) AddOutConnRecord(addr string) {
	logger := log.GetLogger("netserver AddOutConnRecord()")
	...
	logger.Debug().Msgf("[p2p]add out record  %s", addr)
}
...
func (this *NetServer) RemoveFromOutConnRecord(addr string) {
	logger := log.GetLogger("netserver RemoveFromOutConnRecord()")
	this.outConnRecord.Lock()
	defer this.outConnRecord.Unlock()
	addrs := []string{}
	for _, a := range this.outConnRecord.OutConnectingAddrs {
	...
	logger.Debug().Msgf("[p2p]remove out record  %s", addr)
...
func (this *NetServer) AddrValid(addr string) bool {
	logger := log.GetLogger("netserver AddrValid()")
	if config.DefConfig.P2PNodeConfig.ReservedPeersOnly && len(config.DefConfig.P2PNodeConfig.ReservedCfg.ReservedPeers) > 0 {
		for _, ip := range config.DefConfig.P2PNodeConfig.ReservedCfg.ReservedPeers {
			if strings.HasPrefix(addr, ip) {
				logger.Info().Msgf("[p2p]found reserved peer :", addr)
				return true
			}
		}
		return false
...
func (this *NetServer) SetOwnAddress(addr string) {
	logger := log.GetLogger("netserver SetOwnAddress()")
	if addr != this.OwnAddress {
		logger.Info().Msgf("[p2p]set own address %s", addr)
