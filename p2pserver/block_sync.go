...
import (
	...
	p2pComm "github.com/paradigm-network/paradigm/p2pserver/common"
...
var logger = log.GetLogger("p2pserver")
...
func (this *BlockSyncMgr) checkTimeout() {
	...
	for height, flightInfo := range headerTimeoutFlights {
		...
		logger.Info().Msgf("[p2p]checkTimeout sync headers from id:%d :%d timeout after:%d s Times:%d", flightInfo.GetNodeId(), height, SYNC_HEADER_REQUEST_TIMEOUT, flightInfo.GetTotalFailedTimes())
		...
		if err != nil {
			logger.Warn().Msgf("[p2p]checkTimeout failed to send a new headersReq:s", err)
		} else {
	...
	for blockHash, flightInfos := range blockTimeoutFlights {
		for _, flightInfo := range flightInfos {
			...
			flightInfo.ResetStartTime()
			flightInfo.MarkFailedNode()
			logger.Info().Msgf("[p2p]checkTimeout sync height:%d block:0x%x timeout after:%d s times:%d", flightInfo.Height, blockHash, SYNC_BLOCK_REQUEST_TIMEOUT, flightInfo.GetTotalFailedTimes())
			reqNode := this.getNodeWithMinFailedTimes(flightInfo, curBlockHeight)
			if reqNode == nil {
				break
			}
			...
			err := this.server.Send(reqNode, msg, false)
			if err != nil {
				logger.Warn().Msgf("[p2p]checkTimeout reqNode ID:%d Send error:%s", reqNode.GetID(), err)
				continue
			} else {
				this.appendReqTime(reqNode.GetID())
			}
...
func (this *BlockSyncMgr) syncHeader() {
	...
	err := this.server.Send(reqNode, msg, false)
	if err != nil {
		logger.Warn().Msgf("[p2p]syncHeader failed to send a new headersReq")
	} else {
		this.appendReqTime(reqNode.GetID())
	}

	logger.Info().Msgf("Header sync request height:%d", NextHeaderId)
}
...
func (this *BlockSyncMgr) syncBlock() {
	...
	for {
		...
		for t := 0; t < reqTimes; t++ {
			...
			err := this.server.Send(reqNode, msg, false)
			if err != nil {
				logger.Warn().Msgf("[p2p]syncBlock Height:%d ReqBlkData error:%s", nextBlockHeight, err)
				return
			} else {
				this.appendReqTime(reqNode.GetID())
			}
...
func (this *BlockSyncMgr) OnHeaderReceive(fromID uint64, headers []*types.Header) {
	...
	logger.Info().Msgf("Header receive height:%d - %d", headers[0].Height, headers[len(headers)-1].Height)
	...
	if err != nil {
		...
		logger.Warn().Msgf("[p2p]OnHeaderReceive AddHeaders error:%s", err)
		return
	}
	this.syncHeader()
}
...
func (this *BlockSyncMgr) OnBlockReceive(fromID uint64, blockSize uint32, block *types.Block) {
	...
	logger.Info().Msgf("[p2p]OnBlockReceive Height:%d", height)
...
func (this *BlockSyncMgr) OnAddNode(nodeId uint64) {
	logger.Debug().Msgf("[p2p]OnAddNode:%d", nodeId)
	this.lock.Lock()
	defer this.lock.Unlock()
...
func (this *BlockSyncMgr) OnDelNode(nodeId uint64) {
	this.delNode(nodeId)
	logger.Info().Msgf("OnDelNode:%d", nodeId)
}
...
func (this *BlockSyncMgr) delNode(nodeId uint64) {
	...
	logger.Info().Msgf("delNode:%d", nodeId)
	if len(this.nodeWeights) == 0 {
		logger.Warn().Msgf("no sync nodes")
	}
	logger.Info().Msgf("OnDelNode:%d", nodeId)
}
...
func (this *BlockSyncMgr) saveBlock() {
	...
	for {
		...
		if err != nil {
			...
			logger.Warn().Msgf("[p2p]saveBlock Height:%d AddBlock error:%s", nextBlockHeight, err)
			reqNode := this.getNextNode(nextBlockHeight)
			if reqNode == nil {
				return
			}
			...
			err := this.server.Send(reqNode, msg, false)
			if err != nil {
				logger.Warn().Msgf("[p2p]require new block error:", err)
				return
			} else {
				this.appendReqTime(reqNode.GetID())
			}
